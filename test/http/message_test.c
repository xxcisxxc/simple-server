...
void test_parse_right_http_message0() {
    ...
    TEST_ASSERT_EQUAL_STRING("GET /path/file.html HTTP/1.1", (char *)httpMessage.start_line);
    TEST_ASSERT_EQUAL_STRING("Hello World", httpMessage.message_body.body);

    struct header_field *current_header = (struct header_field *)httpMessage.header_field_list.list_head.next->data;
    ...
    current_header = (struct header_field *)current_header->list_head.next->data;
